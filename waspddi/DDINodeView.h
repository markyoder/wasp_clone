#ifndef WASP_DDINODE_VIEW_H
#define WASP_DDINODE_VIEW_H
...
/**
 * @brief The DDINodeView class provides light weight interface to DDI Nodes
...
public:
    ...
    bool operator<(const DDINodeView& b)const;
    /**
     * @brief equal determines if this is equal to the provides DDINodeView
     * @return true, iff and only if the nodes are the same
     */
    bool equal(const DDINodeView& b)const{return *this==b;}
     ...
     * @return DDINodeView describing the child node
     */
    DDINodeView child_at(std::size_t index)const;
    ...
    friend std::ostream& operator<< (std::ostream& str, const wasp::DDINodeView<TNV> & view){
        str<<"DDINodeView(tree_node_index="<<view.m_tree_node_index<<", &pool="<<view.m_tree_data<<")";
        return str;
    }
