...
void display_skeleton(skel_node * current_node, int level){
    ...
    std::cout << std::endl << out_spaces << current_node->skel_name << "{" << std::endl << std::endl;
    
    std::cout     << inn_spaces << "% MinOccurs = 0" << std::endl;
    std::cout     << inn_spaces << "% MaxOccurs = 1" << std::endl;
    if (current_node->skel_children.size() == 0){
        std::cout << inn_spaces << "% ValType   = Int | Real | String "                     << std::endl;
        std::cout << inn_spaces << "% MinValInc = 0"                                        << std::endl;
        std::cout << inn_spaces << "% MaxValExc = NoLimit"                                  << std::endl;
        std::cout << inn_spaces << "% ValEnums  = [ \"choice1\" \"choice2\" \"choice3\" ] " << std::endl;
        std::cout << inn_spaces << "% ExistsIn  = [ \"../some/relative/paths\" ] "          << std::endl;  
    }
    
    for (auto it = current_node->skel_children.begin(); it != current_node->skel_children.end(); it++){
        display_skeleton(it->second, level+1);
    }
    
    std::cout << std::endl << out_spaces << "} % end " << current_node->skel_name << std::endl;
...
int main (int argc, char *argv[])
{
    ...
    if( argc < 2 )
    {
        std::cout<<"Workbench Analysis Sequence Processor (SON) Schema Skeleton Creator"
                <<std::endl
    ...
    for (auto it = skel_root->skel_children.begin(); it != skel_root->skel_children.end(); it++){
