...
int main(int argc, char* argv[])
{
    ...
    if (argc < 3)
    {
        std::cout << "Workbench Analysis Sequence Processor (JSON) Validator"
                  << std::endl
                  << argv[0]
                  << " : An application for validating JSON formatted input."
                  << std::endl;
        std::cout << " Usage : " << argv[0]
                  << " path/to/SON/formatted/schema "
                     "path/to/JSON/formatted/input... [--xml]"
                  << std::endl;
        std::cout << " Usage : " << argv[0]
    ...
    {   
        DefaultJSONInterpreter input_interp(errors);
