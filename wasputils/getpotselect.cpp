...
int main (int argc, char *argv[])
{
    if( argc < 2 )
    {
        std::cout<<"Workbench Analysis Sequence Processor (GetPot)"
                <<std::endl
                <<argv[0]<<" : An application for selecting GetPot formatted input."<<std::endl;
        std::cout<<" Usage : "<<argv[0]<<" path/to/GetPot/formatted/input 'siren select statement'..."
                <<std::endl
    ...
    GetPotInterpreter<TreeNodePool<unsigned int, unsigned int
