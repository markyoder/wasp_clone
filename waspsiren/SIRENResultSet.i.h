...
{
    results.push_back({ADAPTED,static_cast<unsigned int>(adapted_results.size())});
    adapted_results.push_back(node);
}
template<typename TAdapter>
void SIRENResultSet<TAdapter>::push(const std::string& name, const std::string & data)
{
    results.push_back({SCALAR,static_cast<unsigned int>(scalar_results.size())});
