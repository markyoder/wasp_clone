...
TEST( Definition, parent_section)
{
    ...
    ASSERT_EQ( "grandchild2", aname );
    aname = "this will not change because 'agrandchild2' doesn't exist - isn't found";
    ASSERT_EQ(-1, grandchild1->delta("agrandchild2", aname) );
    ASSERT_EQ( "this will not change because 'agrandchild2' doesn't exist - isn't found", aname );
