...
void do_test(const std::string & name)
{
    ...
    HIVE hive;
    GetPotHIVETest t;
    ASSERT_TRUE( load_streams(t,name+".fail.gp"
                              ,name+".pass.gp"
                              ,name+".fail.gld"
                              ,name+".sch") );
    ...
    GetPotNodeView<decltype(t.input_fail_interpreter->root())> input_fail_adapter = t.input_fail_interpreter->root();
    GetPotNodeView<decltype(t.input_pass_interpreter->root())> input_pass_adapter = t.input_pass_interpreter->root();
