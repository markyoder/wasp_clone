...
/**
 * @brief The GetPotNodeView class provides light weight interface to GetPot Nodes
...
public:
     ...
     */
    GetPotNodeView first_non_decorative_child_by_name(const std::string& name)const;
    /**
    ...
    friend std::ostream& operator<< (std::ostream& str, const wasp::GetPotNodeView<TNV> & view){
        str<<"GetPotNodeView(tree_node_index="<<view.m_tree_node_index<<", &pool="<<view.m_tree_data<<")";
        return str;
    }
