...
TEST(TreeNodePool,push_test)
{
    ...
    { // push first line's contents
        ...
        W_ASSERT_EQ(token_data.size(),node_name.size());
        W_ASSERT_EQ(token_data.size(),node_type.size());
        W_ASSERT_EQ(token_data.size(),token_type.size());
        W_ASSERT_EQ(token_data.size(),token_offset.size());
        ...
        for(size_t i = 0; i < token_data.size(); ++i)
        {
            ...
            tp.push_leaf(node_type[i],node_name[i].c_str()
                         ...
                         ,token_data[i].c_str());
            W_ASSERT_EQ(i, tp.leaf_index(i)); // already leaf, so same index
            W_ASSERT_EQ(1, tp.line(i));
            W_ASSERT_EQ(node_type[i],tp.type(i));
            W_ASSERT_EQ(node_name[i],tp.name(i));
            W_ASSERT_EQ(token_offset[i]+1, tp.column(i));
        ...
        W_ASSERT_EQ(7+1, tp.size());
        W_ASSERT_EQ(0, tp.leaf_index(tp.size()-1)); // parent of first leaf node
        W_ASSERT_EQ(1,tp.line(tp.size()-1));
        W_ASSERT_EQ(1,tp.column(tp.size()-1));
        default_file_offset_type_size line_offset
                = token_offset.back()+token_data.back().size();
        W_ASSERT_EQ(25, line_offset)
    ...
    { // push second line
        ...
        for(size_t i = 0; i < token_data.size(); ++i)
        {
            ...
            tp.push_leaf(node_type[i],node_name[i].c_str()
                         ,token_type[i],token_offset[i]
                         ,token_data[i].c_str());
            W_ASSERT_EQ(node_index, tp.leaf_index(node_index)); // already leaf, so same index
            W_ASSERT_EQ(2, tp.line(node_index));
            W_ASSERT_EQ(token_offset[i]-token_offset.front()+1
                        , tp.column(node_index));
            W_ASSERT_EQ(node_index+1, tp.size());
        }
        // add parent node
        std::string parent_name = "data";
        char parent_type = array;
        ...
        root_child_indices.push_back(tp.size()); // capture
        tp.push_parent(parent_type,parent_name.c_str(),child_indices);
        W_ASSERT_EQ(7+1+3+1, tp.size());
        W_ASSERT_EQ(8, tp.leaf_index(tp.size()-1)); // parent of first leaf node
        W_ASSERT_EQ(2,tp.line(tp.size()-1));
        W_ASSERT_EQ(1,tp.column(tp.size()-1));
    }
    std::string root_name = "root";
    tp.push_parent(root,root_name.c_str(),root_child_indices);
    W_ASSERT_EQ(root_name,tp.name(tp.size()-1));
    W_ASSERT_EQ(root, tp.type(tp.size()-1));
}
TEST_END(TreeNodePool,push_test)
